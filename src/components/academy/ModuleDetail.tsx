    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mb-6 rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-md">
        ...
        <p className="mb-4 text-xs text-white/50">
          ...
          {narration.length > 0 && " Or press play and let Brett read it to you."}
        </p>
        {narration.map((track) => (
      ...
      {(m.pdfs.length > 0 || m.images.length > 0) && (
        <div className="relative mb-6 overflow-hidden rounded-2xl border border-gold/30 bg-gradient-to-br from-[#1a1206] via-black to-[#12100a] p-6 shadow-[0_0_40px_-14px_var(--gold)]">
          <span aria-hidden className="pointer-events-none absolute -right-16 -top-16 h-56 w-56 rounded-full bg-gold/15 blur-3xl" />
          <div className="relative">
            ...
            <h2 className="font-heading text-xl font-bold text-white">Resources &amp; Worksheets</h2>
...
function ResourceTile({ label, href }: { label: string; href: string }) {
  ...
  return (
    <div className="group flex gap-4 rounded-xl border border-white/10 bg-white/[0.04] p-4 transition-colors hover:border-gold/50 hover:bg-white/[0.07]">
      {/* Paper sheet */}
      <a href={href} target="_blank" rel="noopener noreferrer" aria-label={`Open ${clean}`} className="shrink-0">
        <div className="relative h-24 w-[72px] rounded-sm bg-gradient-to-b from-[#fbf6ea] to-[#e9dfc6] shadow-[0_8px_20px_-8px_rgba(0,0,0,0.8)] transition-transform group-hover:-translate-y-0.5 group-hover:rotate-[-1.5deg]">
          ...
          <span className="absolute bottom-1.5 left-1.5 rounded bg-cranberry px-1.5 py-0.5 text-[9px] font-black tracking-wider text-white">
            {ext || "FILE"}
          </span>
        </div>
      </a>
      <div className="min-w-0 flex-1">
        <p className="font-heading text-base font-bold leading-snug text-white">{clean}</p>
        <p className="mt-1 text-xs text-white/50">{ext ? `${ext} document` : "Document"} · opens in a new tab</p>
        <div className="mt-3 flex flex-wrap gap-2">
          <a
            href={href}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex min-h-10 items-center gap-1.5 rounded-lg bg-gold px-3.5 text-sm font-heading font-bold text-black hover:bg-gold-light"
          >
          ...
          <a
            href={href}
            download
            className="inline-flex min-h-10 items-center gap-1.5 rounded-lg border border-white/15 px-3.5 text-sm font-semibold text-white/80 hover:bg-white/10 hover:text-white"
          >
