...
export default function PodcastPlayer({ label, src }: { label: string; src: string }) {
  ...
  return (
    <div className="academy-brand relative overflow-hidden rounded-2xl border border-academy-primary/40 bg-gradient-to-br from-[#2a0a12] via-academy-bg to-[#1a1206] p-6 shadow-[0_0_40px_-10px_var(--cranberry)]">
      {/* Ambient glow */}
      <span aria-hidden className="pointer-events-none absolute -left-20 -top-20 h-64 w-64 rounded-full bg-academy-primary/30 blur-3xl" />
      <span aria-hidden className="pointer-events-none absolute -bottom-20 -right-20 h-64 w-64 rounded-full bg-academy-accent/20 blur-3xl" />
      ...
      <div className="relative">
        ...
        <p className="mb-5 font-heading text-base font-bold text-academy-fg sm:text-lg">{label}</p>
          ...
          <div className="flex flex-col items-center gap-3">
            <div
              className={`flex h-14 w-14 items-center justify-center rounded-full border-2 transition-all ${
                playing ? "border-academy-accent bg-academy-accent/20 shadow-[0_0_30px_-4px_var(--academy-accent)]" : "border-academy-fg/20 bg-academy-bg/40"
              }`}
            >
              <Mic size={24} className={playing ? "text-academy-accent" : "text-academy-fg/60"} />
            </div>
            <div className="flex h-8 items-end gap-[3px]" aria-hidden>
              {BARS.map((h, i) => (
                <span
                  key={i}
                  className={`w-[3px] rounded-full bg-gradient-to-t from-academy-primary-light to-academy-accent ${
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex h-11 w-11 items-center justify-center rounded-full text-academy-fg/70 hover:bg-academy-fg/10 hover:text-academy-fg"
          >
            <RotateCcw size={20} />
          </button>
          <button
            ...
            className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-academy-accent text-academy-bg shadow-[0_0_30px_-6px_var(--academy-accent)] transition-transform hover:scale-105 hover:bg-academy-accent-light"
          >
          ...
          <div className="ml-2 flex min-w-0 flex-1 items-center gap-3">
            <span className="w-10 text-right text-xs tabular-nums text-academy-fg/60">{fmt(time)}</span>
            <input
              type="range"
              min={0}
              max={duration || 0}
              ...
              aria-label="Seek"
              className="h-1.5 min-w-0 flex-1 cursor-pointer appearance-none rounded-full bg-academy-fg/15 accent-academy-accent"
              style={{ background: `linear-gradient(to right, var(--academy-accent) ${pct}%, rgba(255,255,255,0.15) ${pct}%)` }}
            />
            <span className="w-10 text-xs tabular-nums text-academy-fg/60">{fmt(duration)}</span>
          </div>
        </div>

        {!playing && time === 0 && (
          <p className="mt-3 text-center text-xs text-academy-fg/50">Press play — two hosts break down this lesson</p>
        )}
      </div>
  ...
  return (
    <div className={`relative ${flip}`} aria-hidden>
      <svg width="72" height="80" viewBox="0 0 72 80" fill="none" className="drop-shadow-[0_4px_12px_rgba(0,0,0,0.6)]">
        ...
        {/* headphones */}
        <path d="M17 32c0-14 8-22 19-22s19 8 19 22" stroke="var(--academy-accent)" strokeWidth="3" strokeLinecap="round" />
        <rect x="13" y="28" width="8" height="14" rx="3" fill="var(--academy-accent)" />
        <rect x="51" y="28" width="8" height="14" rx="3" fill="var(--academy-accent)" />
