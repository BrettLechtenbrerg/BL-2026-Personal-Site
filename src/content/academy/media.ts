...
// Server-only (reads /public at request time). Flashcards live in
//   public/academy/<slug>/flashcards.json   [{ front, back }]
...
const PUBLIC_ACADEMY = path.join(process.cwd(), "public", "academy");
const SLUG_RE = /^[a-z0-9-]+$/;

export async function loadFlashcards(slug: string): Promise<Flashcard[] | undefined> {
  if (!SLUG_RE.test(slug)) return undefined;
  try {
    const raw = await readFile(path.join(PUBLIC_ACADEMY, slug, "flashcards.json"), "utf8");
