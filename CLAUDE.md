...
| Master's Edge Academy | `/academy` | ✅ LIVE (Aug 29 2026) · **Stripe paywall live Sep 8 2026** | Full private LMS: free open signup, per-course Stripe Checkout (Framework $99, Business Tools $499, Clock $499, Book $999) or 100%-off gift codes; ownership in `me_course_access`; all modules open within an owned course. 4 courses / 43 modules, every module has NotebookLM audio + video + flashcards (batch finished Sep 8), written lesson + quiz per module, 80% pass gate, XP + belts, badges, community w/ channels, events calendar, members directory, leaderboard (7/30-day/all-time), certification + certificate. Admin review at `/hub/academy`. Env vars set; keep-alive: Vercel cron daily + GitHub Actions 2×/day, auto-restore. NotebookLM podcast/video/flashcards auto-generated per module (`scripts/academy-notebooklm.mjs`; unattended batch `scripts/academy-batch-ctl.sh`). **The Academy code is the shared Academy Forge engine** (`~/dev/academy-forge`, profile `brands/bl/academy.json`; brand values come from `src/content/academy.config.ts` — GENERATED, never hand-edit; upgrade with `bash ~/dev/academy-forge/install.sh bl`). Site-owned: `modules.ts`, `channels.ts`, `badge-meta.ts`, flashcards, covers, media. **New lessons go through the Lesson Forge** (`scripts/academy-lesson.mjs`, skill `academy`, docs in `~/dev/academy-forge/docs/lms/`) — one sentence → validated `lesson.json` → gate → NotebookLM + narration → deploy. **Read `docs/ACADEMY.md` + `docs/SESSION-NOTES.md`**. |
