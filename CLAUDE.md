...
| Master's Edge Academy | `/academy` | 🟡 Built — needs env vars + real videos | Full private LMS: enrollment-code signup, per-member accounts, 3 modules (video/quiz), 80% pass gate, XP + belt levels, badges, community feed, leaderboard, certification + printable certificate. Admin review at `/hub/academy`. **Read `docs/ACADEMY.md`** (launch checklist: `ACADEMY_ACCESS_CODE` + `ACADEMY_SESSION_SECRET` in Vercel, swap placeholder video ids). |
