# Session Notes â Academy

## Sep 6, 2026 â Channels built (NOT yet deployed)
...
- Vercel Hobby crons are once/day max (deploy REJECTS more) → Vercel cron daily
  (`0 9 * * *`) + GitHub Actions `supabase-keepalive.yml` at 03:00 & 15:00 UTC
  (repo secret CRON_SECRET). Three pings/day across two schedulers.
...
Remaining GHL-parity backlog (agreed order): Events calendar â Members
directory â course cover images + About card â leaderboard 7/30-day tabs.
Then NotebookLM Layer 1 (flashcards + install script), Layer 2 (notebooklm-py
0.8.2 automation â verified Sep 6 it can generate+download audio/video/
flashcards/quiz JSON; unofficial, never load-bearing).

## NEXT SESSION â FIRST ITEM TO DISCUSS
...
1. **Layer 1 (solid, build first)**: flashcard support on Academy modules +
   an install script â Brett downloads any NotebookLM artifact (audio, video,
...
2. **Layer 2 (experiment, build after)**: the unofficial `notebooklm-py`
   library â one command sends a module's lesson text to NotebookLM, waits,
...
- Academy LIVE at /academy (enrollment code EDGE2026), 4 courses, 43 modules,
  ...
  final layout â linear per-course unlock preserved in a comment in
  `unlockedSlugs()` (src/content/academy/modules.ts).
- Framework course (module 43, from book Appendix A) is the planned **free
  giveaway** â NOT yet public, still behind the enrollment code by Brett's
...
- Module 43 has BOTH NotebookLM artifacts installed and verified streaming:
  ...
  Page order (Brett's request): Video Overview â Listen â YouTube placeholder
  â written lesson. Both video players stay until Brett picks one.
